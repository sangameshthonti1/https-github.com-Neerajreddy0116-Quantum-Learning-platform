...
export default function BlochSphere({ step, beginner = false }: { step: TraceStep | null; beginner?: boolean }) {
  ...
  return <section className="bloch-panel" aria-label={beginner ? 'Qubit state map' : 'Reduced qubit states'}>
    ...
    {!reduced || !step ? <p className="lab-muted">Trace the current circuit and select a step to inspect its reduced qubit states.</p> : <>
      ...
      <p className="lab-muted">Step {step.index} · q{reduced.qubit}{!beginner && <> · <strong>{mixedness}</strong></>}</p>
      <svg className="bloch-sphere" viewBox="0 0 280 280" role="img" aria-labelledby={`${id}-title ${id}-description`}>
        ...
        <desc id={`${id}-description`}>Step {step.index}. {beginner ? 'The arrow maps the qubit state.' : mixedness} Vector ({formatNumber(vector!.x)}, {formatNumber(vector!.y)}, {formatNumber(vector!.z)}). Length {formatNumber(length)}. Dashed segments are behind the center plane.</desc>
      ...
      {beginner ? <p className="lab-muted" data-testid={`bloch-values-q${reduced.qubit}`}>Arrow coordinates (X, Y, Z): ({formatNumber(vector!.x)}, {formatNumber(vector!.y)}, {formatNumber(vector!.z)}). Each number is how far the arrow extends along that axis. (1, 0, 0) points along +X; (0, 0, 1) points up along +Z.</p> : <><dl className="bloch-values" data-testid={`bloch-values-q${reduced.qubit}`}>
      ...
      <p className="lab-muted">{beginner ? 'Rotating this picture changes only your viewing angle, not the qubit’s state.' : 'The arrow uses the backend’s reduced-state vector. Camera rotation does not change the quantum state.'}</p>
    </>}
  </section>;
}
