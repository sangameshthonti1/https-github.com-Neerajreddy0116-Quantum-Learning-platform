...
import CircuitCanvas, { type PendingPlacement } from './CircuitCanvas';
import CodeMode from './CodeMode';
import { arity, gateDefinitions, makeGate } from './gates';
import { validAngle } from './AngleField';
...
export default function CircuitLab({ experiment = null, initialExploring = false, challenge, nextChallenge, algorithm }: { experiment?: WorkspaceExperiment | null; initialExploring?: boolean; challenge?: Challenge; nextChallenge?: Challenge; algorithm?: AlgorithmWorkspace }) {
  ...
  const applyTutorCircuit = useCallback((next: SimulationRequest) => {
    dispatch({ type: 'replace', request: next }); setSelectedId(null); setPending(null); setPlacementError(null);
  }, [dispatch]);
  ...
  useEffect(() => { setPending(null); setPlacementError(null); }, [request]);

  function chooseTool(next: Gate['type']) {
    setExploring(false); setCodeMode(false);
    setTool(next); setSelectedId(null); setPending(null); setPlacementError(null);
  }
  function addGate(gate: Gate, index: number) {
    setExploring(false); setCodeMode(false);
    dispatch({ type: 'insert', gate, index }); setSelectedId(gate.id); setPending(null); setMobilePanel('circuit');
  }
  function deselect() {
    setSelectedId(null); setPending(null); setPlacementError(null);
  ...
  function drop(type: Gate['type'], qubit: number, index: number) {
    ...
    place(type, qubit, index, null);
  }
  function cell(qubit: number, index: number) {
    place(tool, qubit, index, pending);
  }
  function place(type: Gate['type'], qubit: number, index: number, started: PendingPlacement | null) {
    setPlacementError(null);
    if (gateDefinitions[type].parameterized && !validAngle(angle)) {
      setPlacementError('Enter a finite angle in radians or choose a preset before placing a gate.');
    } else if (request.numQubits < arity(type)) {
      setPlacementError(`${type.toUpperCase()} needs ${arity(type)} qubits. Add a qubit first.`);
    } else if (started && (started.qubits.includes(qubit) || started.index !== index)) {
      setPlacementError(type === 'swap' ? 'Choose a different target qubit in the same step as the first wire.' : 'Choose a different target qubit in the same step as the control.');
    } else {
      const qubits = [...(started?.qubits ?? []), qubit];
      if (qubits.length === arity(type)) addGate(makeGate(type, qubits, Number(angle)), index);
      else { setSelectedId(null); setPending({ type, qubits, index }); }
    }
  }
  async function run() {
    if (active.current || shotsPending || pending) return;
  ...
  return <main className={`lab${challenge ? ' challenge-lab' : ''}`} onKeyDown={(event) => {
    ...
    <div className="lab-layout" data-mobile-panel={mobilePanel} data-code={codeMode && !exploring} data-exploring={exploring} data-explorer-pane={explorerPane}>
      ...
      <section className="lab-workspace" ref={workspace}>
        {legacy && <LessonLabGuide experiment={legacy} request={request} result={result} trace={trace} exploring={exploring}
          busy={loading || trace.loading || shotsPending || pending !== null} failed={error !== null}
        ...
        <div className="workspace-modes" aria-label="Workspace mode">
          <button aria-label="Circuit editor" aria-pressed={!exploring && !codeMode} onClick={() => { setExploring(false); setCodeMode(false); }}>Visual</button>
          <button aria-pressed={!exploring && codeMode} onClick={() => { setExploring(false); setCodeMode(true); setSelectedId(null); setPending(null); }}>Code</button>
        ...
        <CircuitCanvas request={request} selectedId={selected?.id ?? null} tool={tool} pending={pending} onCell={cell} dragTool={dragTool} onDrop={drop}
          ...
          onSelect={(id) => { setExploring(false); setCodeMode(false); setSelectedId(id); setPending(null); setPlacementError(null); }} onDeselect={deselect} onCancel={() => { setPending(null); setPlacementError(null); }} />
        <div hidden={!codeMode || exploring}><CodeMode request={request} workspaceKey={workspaceKey} onApply={applyTutorCircuit} /></div>
        <div hidden={exploring || codeMode}>
        {selected ? <GateInspector request={request} selected={selected}
          ...
          : <details className="lab-insertion" key={tool}>
            ...
            <GateForm key={`${tool}:${request.numQubits}:${request.gates.length}:${angle}`} request={request} selected={undefined} tool={tool} onSave={addGate} defaultAngle={angle} />
          </details>}
        <details className="lab-workspace-help"><summary>How to use the circuit</summary>
          <p>Select a gate, then click or tap a + cell. On desktop, you can also drag a gate from the palette to an empty cell. For CX and CZ, place the control first; for CCX place both controls, then the target. For SWAP choose two wires. Use distinct wires at the same step.</p>
        ...
        {challenge && <ChallengeGrading challenge={challenge} request={request} blocked={shotsPending || pending !== null} nextChallenge={nextChallenge}
          onSubmitting={() => setSelectedId(null)}
          onInspect={() => { setExploring(true); setSelectedId(null); setMobilePanel('circuit'); void trace.run(); }} />}
        {exploring && <StateExplorer trace={trace} blocked={shotsPending || pending !== null} pane={explorerPane} onPane={setExplorerPane} />}
      </section>
