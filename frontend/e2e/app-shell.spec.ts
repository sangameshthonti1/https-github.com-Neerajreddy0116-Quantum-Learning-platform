...
const section = (page: Page, index: number) =>
  page
...
test("dashboard and curriculum use real availability and do not create lesson activity", async ({
  page,
}, info) => {
  const errors: string[] = [];
  ...
  await page.goto("/");
  await expect(
    page.getByRole("link", { name: "Start your quantum journey" }),
  ).toBeVisible();
  await expect(page.getByRole("progressbar")).toHaveAttribute("value", "0");
  expect(
    await page.evaluate(() => sessionStorage.getItem("qlp-superposition-v1")),
  ).toBeNull();
  await button(page, "01 prepare").click();
  await expect(
    page.getByRole("img", { name: /Conceptual map: the zero state/ }),
  ).toBeVisible();
  await button(page, "03 observe").click();
  await expect(
    page.getByText("Measuring |+⟩ gives one bit: 0 or 1, with equal chances."),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("link", { name: "Start lesson", exact: true }),
  ).toHaveCount(4);
  await button(page, "Available now 4").click();
  await expect(page.getByRole("article")).toHaveCount(4);
  await button(page, "All topics 4").click();
  await page.screenshot({
    path: info.outputPath("curriculum-desktop.png"),
    fullPage: true,
  });
  await nav(page, "Progress");
  await expect(page.getByRole("progressbar")).toHaveAttribute("value", "0");
  await expect(
    page.getByText("No lesson activity yet.", { exact: false }),
  ).toHaveCount(4);
  expect(
    await page.evaluate(() => sessionStorage.getItem("qlp-superposition-v1")),
  ).toBeNull();
...
for (const [route, heading, active] of [
  ["/", /Quantum makes sense/, "Dashboard"],
  ["/dashboard", /Quantum makes sense/, "Dashboard"],
  ["/learn/", /Big ideas/, "Learn"],
  ["/learn/superposition", /Superposition and/, "Learn"],
  ["/library", /reference you can read/, "Library"],
  ["/library/unlock", /Unlock the complete Quantum Library/, "Library"],
  ["/payments", /Payments & billing/, "Payments"],
  ["/lab", /^Circuit Lab$/, "Circuit Lab"],
  ["/lab/states", /^Circuit Lab$/, "Circuit Lab"],
  ["/algorithms", /Algorithms, built/, "Algorithms"],
  ["/challenges", /Put your intuition/, "Challenges"],
  ["/progress", /Every observation/, "Progress"],
] as const)
  test(`direct route ${route} loads with correct active navigation`, async ({
    page,
  }) => {
    ...
    if (route === "/lab/states")
      await expect(button(page, "State Explorer")).toHaveAttribute(
        "aria-pressed",
        "true",
      );
    if (route === "/algorithms") {
      ...
      await expect(page.getByText("Experimental", { exact: true })).toHaveCount(
        2,
      );
    }
    if (route === "/challenges")
...
for (const width of [320, 390, 768])
  test(`navigation and layout remain accessible at ${width}px`, async ({
    page,
  }, info) => {
    ...
    await page.screenshot({
      path: info.outputPath(`dashboard-${width}.png`),
      fullPage: true,
    });
    await nav(page, "Learn");
    await expect(
      page.getByRole("heading", { level: 1, name: /Big ideas/ }),
    ).toBeVisible();
    await noOverflow(page);
    await page.screenshot({
      path: info.outputPath(`curriculum-${width}.png`),
      fullPage: true,
    });
...
test("collapse preference survives reload and focused workspace expansion does not squeeze the canvas", async ({
  page,
}) => {
  await page.goto("/");
...
test("free circuit and undo history survive navigation, browser history, and draft reload", async ({
  page,
}) => {
  await page.goto("/lab");
  await button(page, "Choose X gate").click();
  await button(page, "Place gate on q1 at step 1").click();
...
test("guided exercise drafts are isolated from free exploration and both can be resumed", async ({
  page,
}) => {
  await page.goto("/lab");
  await button(page, "Choose X gate").click();
  ...
  await page
    .getByRole("article")
    .filter({ hasText: "Superposition & the Hadamard gate" })
  ...
  await expect(
    page.getByRole("link", { name: "Return to lesson", exact: true }).first(),
  ).toBeVisible();
  await page.getByRole("link", { name: "Open free exploration" }).click();
  await expect(button(page, "Select X gate at step 1")).toBeVisible();
  await expect(
    page.getByRole("region", { name: "Lesson experiment" }),
  ).toHaveCount(0);
  await nav(page, "Learn");
  await page
...
for (const mobile of [false, true])
  test(`real ${mobile ? "mobile" : "desktop"} Dashboard → Learn → Lesson → Lab → State Explorer → lesson journey`, async ({
    page,
  }, info) => {
    if (mobile) {
    ...
    await page
      .getByRole("radio", { name: "50% zero / 50% one", exact: true })
      .check();
    await button(page, "Submit prediction").click();
    await section(page, 5).click();
    await page
      .getByRole("link", { name: "Open the Lab: build one H →" })
      .click();
    await button(page, "Choose H gate").click();
    await button(page, "Place gate on q0 at step 1").click();
    ...
    await expect(page.getByTestId("trace-probability-0")).toContainText(
      "100.0000%",
    );
    ...
    await expect(
      page.getByRole("img", { name: /Bloch sphere for q0/ }),
    ).toBeVisible();
    await page.screenshot({
      path: info.outputPath("journey-explorer.png"),
      fullPage: true,
    });
    await button(page, "Collect experiment & return").click();
    await expect(
      page.getByRole("heading", { name: "Read your results", exact: true }),
    ).toBeVisible();
    ...
    const saved = await page.evaluate(() =>
      JSON.parse(sessionStorage.getItem("qlp-superposition-v1")!),
    );
    expect(saved.evidence.h.simulation).toEqual(body);
    expect(saved.predictions.h).toBe(1);
    await page
      .getByRole("link", { name: "Continue lesson", exact: true })
      .last()
      .click();
...
test("blocked storage retains lesson and circuit during client navigation", async ({
  page,
}) => {
...
test("invalid saved free circuit recovers to a valid empty workspace and unknown paths show recovery", async ({
  page,
}) => {
  await page.addInitScript(() =>
    sessionStorage.setItem(
      ...
      JSON.stringify({
        ...
        numQubits: 1,
        shots: 1024,
        gates: [{ type: "invented", id: "x", targets: [8], controls: [] }],
      }),
    ),
  );
  await page.goto("/lab");
  await expect(button(page, "Place gate on q0 at step 1")).toBeVisible();
  await expect(button(page, "Place gate on q1 at step 1")).toBeVisible();
  await page.goto("/not-a-real-page");
  await expect(
    page.getByRole("heading", { name: "This page is outside our orbit." }),
  ).toBeVisible();
